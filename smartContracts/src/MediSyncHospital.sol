...
contract MediSyncHospital {
    ...
    modifier isAdmin() {
        require(
            msg.sender == adminstrator[0] ||
                msg.sender == adminstrator[1] ||
                msg.sender == adminstrator[2],
            "NOT_ADMIN"
        );
        _;
    }
    ...
    function storePatientBio(
        ...
        string memory choose_passkey
    ) public {
        //check if patient has record. else, create record
        require(
            IMediSyncRecords(recordsContractAddr).confirmBioRecord(msg.sender),
            "RECORD_EXISTS"
        );
        patientBio memory bio = patientBio(
            msg.sender,
            _firstName,
            _lastName,
            _age,
        ...
        bytes32 hashRes = keccak256(abi.encodePacked(choose_passkey));
        IMediSyncRecords(recordsContractAddr).bioEntry(
            _calldata,
            msg.sender,
            hashRes
        );
    ...
    function bookDoctorSession(
        ...
        IMediSyncRecords(recordsContractAddr).healthRecordBySessionId(
            sessionIDgenerator,
            _calldata,
            msg.sender
        );
        requestedSessionInfo[doctors_address].push(
            doctorSessionNote(msg.sender, sessionIDgenerator)
        );
        sessionIDgenerator++;
    ...
    function registerDoctor(
        ...
        (uint _positionInArray, uint doctorID) = IMediSyncFactory(
            factoryAddress
        ).registerDoctors(_calldata);
        isDoctorRegistered[_doctorAddress] = true;
        doctorAddressToID[_doctorAddress] = doctorID;
        doctorIDtoAddress[doctorID] = _doctorAddress;
        doctorDetails.positionInArray = _positionInArray;
        doctorDetails.doctorsID = doctorID;
        HospitalDoctors.push(doctorDetails);
        //to do: assign key
    }
    ...
    function handleConsultation(
        ...
        (, sessionDetails memory session_, ) = retrieveRecord(
            _patient,
            _sessionID,
            _passkey
        );
        require(
            session_.bookedDoctorID == doctorAddressToID[msg.sender],
            "Not_assigned_doctor"
        );
    ...
    function retrieveRecord(
        ...
        internal
        returns (
            patientBio memory bioData,
            sessionDetails memory currentSessionDetail,
            sessionDetails[] memory prevSessions
        )
    {
        (
            bytes memory bio_,
            bytes memory session_,
            bytes[] memory sessionHistories
        ) = IMediSyncRecords(recordsContractAddr)
                .retrieveHealthRecordBySessionId(
                    _sessionID,
                    _patientAddress,
                    _key
                );
    ...
    function checkBookedSessions(
        address _doctor
    ) public view returns (doctorSessionNote[] memory) {
        return requestedSessionInfo[_doctor];
    }

    function previewMedicalData(
        address _patient,
        uint _sessionID,
        string memory _passkey
    )
        ...
        returns (
            patientBio memory,
            sessionDetails memory,
            sessionDetails[] memory prevSessions
        )
    {
        (
            patientBio memory bio_,
            sessionDetails memory session_,
            sessionDetails[] memory prevSessions_
        ) = retrieveRecord(_patient, _sessionID, _passkey);
        return (bio_, session_, prevSessions_);
    }

    function PatientMedicalPreview(
        address patient,
        string memory _passKey
    ) public view returns (bytes memory, bytes[] memory) {
        (bytes memory bio_, bytes[] memory sessionHistories) = IMediSyncRecords(
            recordsContractAddr
        ).retrieveMedicalHistory(patient, _passKey);
        //abi.decode data
    }
}
